...
Transformers operate on sequences of vectors. Text is already a sequence (bytes or tokens). An image is a 2D grid of pixels with three color channels — not a sequence. If you flatten every pixel, a $224 \times 224$ RGB image becomes 150,528 tokens, and self-attention at that length is a non-starter (quadratic in sequence length).

Pre-2020 approaches bolted a CNN feature extractor onto the front: ResNet produces a $7 \times 7$ feature map of 2048-dim vectors, feed those 49 tokens to a transformer. This works but inherits the CNN's biases (translation equivariance, local receptive fields) and loses the transformer's appetite for scale.

Dosovitskiy et al. (2020) asked the blunt question: what if we skip the CNN? Split the image into fixed-size patches (say $16 \times 16$ pixels), linearly project each patch into a vector, add a positional embedding, and feed the sequence to a vanilla transformer. At the time this was heresy — vision without convolutions. With enough data (JFT-300M, then LAION) it beat ResNet on ImageNet and kept improving.
...
$$patch_{\text{embed}} = 3 \cdot 16 \cdot 16 \cdot 768 + 768 = 591k$$
$$cls + pos = 768 + 197 \cdot 768 = 152k$$
$$block = 4 \cdot 768^{2} (QKVO) + 2 \cdot 4 \cdot 768^{2} (MLP) + 2 \cdot 2 \cdot 768 (LN)$$
$$= 12 \cdot 768^{2} + 3k = 7.1M$$
$$12 blocks = 85M$$
$$final LN = 1.5k$$
total       ≈ 86M
...
- Token sequence for a synthetic $8 \times 8$ pixel toy image (walk through the flatten + project path).
...
1. Compute the patch-token sequence length for Qwen2.5-VL at native $1280 \times 720$ input with patch size 14. How does that compare to a CLS-only representation?

2. A 1080p frame ($1920 \times 1080$) at patch 14 produces how many tokens? At 30 FPS over a 5-minute video, how many total visual tokens? Which cost saves you most: pooling, frame sampling, or token merging?
...
|------|----------------|------------------------|
| Patch | "$16 \times 16$ pixel square" | A fixed-size non-overlapping region of the input image; becomes one token |
...
- [Dosovitskiy et al. — An Image is Worth $16 \times 16$ Words (arXiv:2010.11929)](https://arxiv.org/abs/2010.11929) — original ViT.
