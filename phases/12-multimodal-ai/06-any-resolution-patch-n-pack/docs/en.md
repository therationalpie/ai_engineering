...
Transformers expect a sequence. A batch is a stack of sequences the same length. If your images are $224 \times 224$, you get 196 patch tokens every time, padding not required, job done. Train on 224, infer on 224, never think about resolution again.

The world does not cooperate. Documents are portrait ($8.5 \times 11$ inches, 2:3-ish). Chart screenshots are landscape (16:9). Receipts are tall and thin (1:3). Medical imaging ships at $2048 \times 2048$ or larger. Mobile device screenshots are $1170 \times 2532$ (0.46:1).

Three pre-2024 options and why each fails:

1. Resize to a fixed square ($224 \times 224$ or $336 \times 336$). The squish distorts text and faces. The downscale destroys chart labels and OCR content. Standard practice until LLaVA-1.5.
...
A batch of three images at $336 \times 336$ (576 tokens), $224 \times 224$ (256 tokens), and $448 \times 336$ (768 tokens) becomes one 1600-token sequence with a $1600 \times 1600$ block-diagonal mask. No padding. No wasted compute. The transformer handles arbitrary aspect ratios.
...
1. Pick a grid layout from a predefined set — ($1 \times 1$), ($1 \times 2$), ($2 \times 1$), ($1 \times 3$), ($3 \times 1$), ($2 \times 2$), etc. — that best fits the image's aspect ratio.
2. Tile the full image into the grid; each tile becomes a $336 \times 336$ crop.
3. Also produce a thumbnail: the whole image resized to $336 \times 336$ as a global-context token.
4. Encode every tile through the frozen 336-encoder. Concatenate the tile tokens + thumbnail tokens.

For a $672 \times 672$ image at $2 \times 2$ grid plus thumbnail: 4 * 576 + 576 = 2880 visual tokens. Expensive but effective — the LLM sees both local detail and global context.

AnyRes is the route of choice when your encoder is frozen and only supports one resolution. It explodes token count for large images (a $1344 \times 1344$ image at $4 \times 4$ grid is 9216 + 576 ≈ 9800 tokens, which fills most of a 8k LLM context).
...
$$offsets = [0, n_{0}, n_{0}+n_{1}, \ldots, N_{\text{total}}]$$
$$M[i, j] = 1 iff there exists b where offsets[b] \le i < offsets[b+1] and offsets[b] \le j < offsets[b+1]$$
...
- OCR / documents: 1024-4096 tokens. SigLIP 2 NaFlex at 1024, or AnyRes $3 \times 3$ + thumbnail.
...
1. A receipt is $600 \times 1500$ (1:2.5). At patch size 14, how many native-resolution tokens? How many after square-resize to 336? Which loses more OCR accuracy in practice?
...
3. For a $1792 \times 896$ image at patch 14, compare: (a) square-resize to 336 then encode, (b) AnyRes $2 \times 1$ + thumbnail, (c) M-RoPE at native. Which uses fewest tokens? Which preserves most detail?
